...
%% YARDA3: link na http://www.crk.cz/CZ/KONCEC 
...
%% YARDA2 (DONE): je to požadavek, takže by to mělo být dále formulováno podobně
...
Aplikace bude tedy složena z modulárního serveru, klientské knihovny a grafického uživatelského rozhraní.
...
Pro jednoduchost se pro přenos užitečných dat využije formátu CSV \cite{csv}. První řádek
bude reprezentovat hlavičku dat. Názvy jednotlivých
sloupců byly zvoleny následovně: %% YARDA2 (DONE): opět, pokud je kapitola nadepsaná
...
%% YARDA3: navrhuji změnit název kapitoly na případy použití
...
Díky tomu se nebude muset heslo posílat při přihlášení po sítí %% YARDA2 (DONE): opět návrh, tedy: .. heslo se numusí
%% posílat po síti 
...
V rámci této bakalářské práce budou uvažovány moduly následujících typů (lze
...
DXCluster, tedy budou poskytovat informace (například frekvenci a polohu) o
aktuálně vysílajících stanicích.
\end{itemize}
...
V tabulce logbook budou uloženy deníky všech uživatelů. Pro uložení dat byly
...
Klientská knihovna poskytne grafickému rozhraní jednotné API pro přístup k
serveru. To zamezí duplikaci kódu při případné implementaci více
...
Klient také umožní zobrazit všechny moduly dostupné na serveru včetně jejich
textového popisu.
...
\section{Protokol}
%% YARDA3: doporučuji rozhodit do kapitoly implementované moduly 
