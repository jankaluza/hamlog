...
%% YARDA: Přidat úvod - rozvinout abstrakt, aby čtenář věděl o čem to bude.
%% Strukturu práce pak buďto popsat přimo v tom textu nebo v dalším odstavci nebo přidat podkapitolu
%% Člěnění práce, nebo struktura práce, atp. Popis kapitol bych asi
%% nestrukturoval, popsal bych to volně navazujícími větami, např.
%% První kapitola obsahuje úvod, dále následuje kapitola popisující stručně
%% historii amatérské radiokomunikační služby, atp.
...
%% unie. Naše země stála u jejího zrodu jako součást rakouského mocnářství. ITU
%% je dnes stálou mezinárodní organizací při OSN pro dálkové telekomunikace a
%% správu kmitočtového spektra. Opatření ITU jsou v ČR právně závazná z titulu členství ČR v ITU a v
%% mezích základních dokumentů ITU.

%% YARDA: mám trochu problém se slovem rádio, podle mě je to slangový výraz
...
Prvotní motivací pro vznik amatérského rádia%% YARDA: Tady bych dal např. amatérského vysílaní atd.
byla myšlenka našich předků rádio %% YARDA: Asi radio jako mají na ČRK.
...
Amatérské rádio se začalo rychle šířit po první světové válce hlavně díky vlivu
rozhlasu. %% YARDA: tady bych byl konzistentní v pojmech, komerční radiové
...
%% YARDA: Přidat následující (důležité):
...
pro dálkový příjem/vysílání. Pocházi z anglického "distance
...
Echolink %%YARDA: link literatura, na echolink.org,
...
Tato činnost je koordinována neziskovou organizací AMSAT. %% YARDA: link na AMSAT

%% YARDA: konzistentní terminologie, družice vs satelit

%% YARDA: toto se mi nezdá, jak je to myšleno? zdroj? možná vypustit
...
Výhodou morseovy %% YARDA: asi bych dal velké M
...
přenosu hlasu, např. D-STAR, %% YARDA: link
P25 C4FM, atp.
...
jeho přibližnou polohu. K tomu slouží seznam DXCC (DX Century Club)%% YARDA: link
...
\section{Logger32}%% YARDA: logovací aplikace nahradit staniční deník
...
zalogovat %% YARDA: místo zalogovat používat např. zapsat
...
Delphi. %% YARDA: zmínit že to pro kompilaci potřebuje Lazarus 
...
%% YARDA: zmínit linux princip KISS (keep it simple and stupid), tedy modulární
...
\caption{Základní činnost programu.} %% YARDA: možná raději základní
...
%% napsat pro lamy proč komunikační protokol, k čemu ho vlastně potřebujeme.
...
%% YARDA: taky napsat výhodu prostupnost přes firewally proxy, atp.
...
Protokol je zpětně kompatibilní s protokolem HTTP, %% YARDA: jestli to projde
%% přes standardní HTTP proxy, tak to zmínit, viz výše
...
\item \textbf{mode} - Mód spojení. %% YARDA: příklad
\item \textbf{qth} - QTH lokátor (Maidenhead Locator).%% YARDA: závorka není
%% třeba, bylo vysvětleno výše
...
%% YARDA: specifikoval bych výčtem, co všechno ten protokol musí umět a poskytovat,
...
zahashované. %% YARDA: škaredé slovo přeformulovat a použít např. slovo 'otisk'
...
Server je konzolová aplikace %% YARDA: místo konzolová aplikace raději aplikace
...
v INI formátu. %% YARDA: popsat co to je, nebo link 
...
\item \textbf{cq} - CQ zóna.
\item \textbf{itu} - ITU zóna.
\item \textbf{qsl} - Příznak žádosti o QSL lístek. %% YARDA: rozvést proč je to
%% použito a co to umožňuje (např. dále v textu).
...
rozhraním. %% YARDA: blíže specifikovat, co se myslí, QT, GTK?
...
Je u ní použit návrhový vzor Singleton %% YARDA: více rozvést, nebo link do
%% literatury
...
příznaky pro NOT NULL, UNIQUE a PRIMARY KEY). %% YARDA: opět rozvést
...
(pure virtual) metody. %% YARDA: hodil by se odkaz na nějakou knihu o OOP 
Veškeré klientské požadavky jsou pak směrovány na konkrétní modul podle URI instancí třídy ModuleManager.

%% YARDA: každý obrázek musí být odkazován v textu, např. Na obrázku
...
\subsection{Seznam implementovaných modulů} %% YARDA: možná raději
...
Modul Register (URI "/register") %% YARDA: 'běžící na' bych odstranil
...
implementován princip přihlášení WWW-Authenticate. %% YARDA: určitě link
...
\begin{itemize}
\item URI "/logbook" - Jako odpověd na dotaz pošle celý logbook %% YARDA:
...
byl získán z TODO. %% YARDA: doplnit
...
%% YARDA: rozvést, popsat stručně hamlib, minimálně link
V metodě handleRequest se na základě
URI provádí tyto akce:

\begin{itemize}
\item dotaz typu POST na URI "/hamlib" - Přeladí frekvenci vysílačky %% YARDA:
...
%% YARDA: pokud se jedná o kapitolu implementace, tak jednotlivé podkapitoly
...
%% YARDA: opět konzistence, předchozí podkapitoly obsahují slovo implementace,
...
%% YARDA: dát to do kontextu, k čemu se to používá
...
%% YARDA: opět dát to do kontextu, k čemu se to používá
...
hashovacího algoritmu. %% YARDA: určitě link
...
%% YARDA: někde do textu dát, že komunikace s radiostanicí byla prakticky
...
%% YARDA: vypíchnout, že se to může použít např. pro automatické generování logu
%% na WWW stránky, atp.
...
%% YARDA: opět vypíchnout, že se předpokládá vznik dalších klientů, např. pro
...
%% YARDA: pochlubit se, že se jedná o opensource projekt uvolněný pod GPL, který
...
je díky současnému návrhnu jednoduché. %% YARDA: možná se zmínit, že v
...
%% YARDA: více rozvést, že lze konstruovat kompletní dotazy a tedy spočítat
